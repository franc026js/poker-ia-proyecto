...
function reiniciarPartida() {
  // Restablece el estado del jugador a la configuración inicial
  ...
  mostrarMensajeGrande("Partida reiniciada<br>Dinero: $" + DINERO_INICIAL);
  resultadoHTML.textContent = "Partida reiniciada con $" + DINERO_INICIAL + ".";
  actualizarPantalla();
}
...
async function repartirCartas() {
  if (partidaActiva || repartiendo || dinero <= 0) {
    if (dinero <= 0) {
      ...
      // Si ya usaste préstamo y no puedes jugar, reiniciar la partida automáticamente
      if (prestamoUsado && !partidaActiva && !repartiendo) {
        mostrarMensajeGrande("Te quedaste sin dinero. Reiniciando partida...");
        setTimeout(reiniciarPartida, 1200);
      }
    }
    return;
  }
...
function fold() {
  ...
  // Si tras terminar la mano te quedaste sin dinero y ya habías pedido préstamo,
  // reiniciamos la partida automáticamente para evitar quedar bloqueado.
  if (dinero <= 0 && !partidaActiva) {
    if (prestamoUsado) {
      mostrarMensajeGrande("Te quedaste sin dinero. Reiniciando partida...");
      setTimeout(reiniciarPartida, 1200);
    } else {
      resultadoHTML.textContent = "Te quedaste sin dinero. Puedes pedir un préstamo.";
    }
  }
}
